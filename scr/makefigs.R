...
p3a <-d %>%  filter(estimate=="Dirichlet") %>% ggplot(aes(x, y, fill=probability)) + geom_tile() +
  scale_fill_gradient2() + ggtitle("Dirichlet") + xlab("") + ylab("") + theme(aspect.ratio=1)

p3b <- d %>%  filter(estimate=="graphLaplacian") %>%ggplot(aes(x, y, fill=probability)) + geom_tile()+
  scale_fill_gradient2() + ggtitle("graphLaplacian")+ xlab("") + ylab("") + theme(aspect.ratio=1)
grid.arrange(p3a,p3b, ncol=2)

pdf("./out/obs.pdf",width=5,height=5)
 ggplot() + geom_tile(data=d, aes(x,y,fill=ptrue)) + scale_fill_gradient2()+ ggtitle("true bin probabilities and observations")+
  xlab("") + ylab("") + theme(aspect.ratio=1) + geom_point(data=obs, aes(x=t,y=y, colour=yobserved), size=1.1)
dev.off()