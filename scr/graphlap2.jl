...
@model GraphLaplacianModel2(ci,L,nsample) = begin
    τ ~ InverseGamma(.1,.1)
    H ~ MvNormalCanon(L*τ)
    Turing.@addlogprob! loglik(ci, H, nsample)
end

function loglik(ci, H, nsample)
    θ = logistic.(H)
    ll = - nsample * log(sum(θ))
    @inbounds for i ∈ 1:nsample
        γ = ci[k]
...
function sample_graphlap(t,ind_yknown, y,binx,biny, ITER_GL, BI_GL; sampler=HMC(0.1, 5))
    ...
    model = GraphLaplacianModel2(ci,L,NSAMPLE)
