...
function plotting_d(d, titel;mincol_lim=-1,maxcol_lim=1)
    ...
    R"""
        ...
        library(tidyverse)
        theme_set(theme_light())
         p <- ggplot(data=d,aes(x, y, fill=dest-dtrue)) + geom_tile() +
                scale_fill_gradient2(limits=c(mincol_lim, maxcol_lim))+
        ggtitle(paste0(titel," - density error"))
        #ggsave(paste0("./out/",titel,"_d.pdf"))
        pdf(paste0("./out/",titel,"_d.pdf"),width=8,height=4)
            show(p)
        dev.off()
    """
    ...
    R"""
        ...
        library(tidyverse)
        theme_set(theme_light())
        p<-ggplot(data=d,aes(x, y,z=dtrue)) +
        ...
        pdf("./out/truedensitydensity.pdf",width=8,height=4)
            show(p)
        dev.off()
    """
